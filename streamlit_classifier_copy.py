import base64
import helpers.classifier_helper_copy as ch
...
def streamlit_main():
    ...
    # print(directory)
    #resume_classifier-v0.001-01

    classifier_model_files = [f.strip('resume_classifier-v').strip('.pkl') for f in os.listdir(classifier_directory) if f.endswith(".pkl")]
   
    if not classifier_model_files:
        st.error("No model files found in the directory.")
        return
    
    vectorizer_model_files = [f.strip('vectorizer-').strip('.pkl') for f in os.listdir(vectorizer_directory) if f.endswith(".pkl")]
